...
int main(int argc, char** argv) {
	...
	void main() {
		...
		float trans1 = clamp(secondsAbs - 3,0,1000);
		float trans2 = clamp(secondsAbs - 4,0,1);
		rotationY = rotationY*trans2;
	...
	string buf = get_file_contents("songs/jk_whatever_part.mod");
	...
	for(int i=0;i<N;i++) {
		...
		if(n.channel == 10 && n.chordCount == 2) {
			Note newNote = n;
			newNote.channel = 100;
	...
	for(int i=0;i<(int)notes.size();i++) {
		...
		if(n.channel <= 3) {
			// chords
			srcName = selectSource("wind", 1); n.amplitudeDB -= 8;
			fade = 0.;
			reverbWet = 0.2;
			
			sz = 1./5 * 0.95;
			y = 1. - sz/2*1.1;
			x = (n.channel-1.5)*sz + 0.5;
			sz *= 0.85;
		} else if(n.channel == 9) {
			// bass
			...
			// main melody
			if(n.start.absRow < 4*64+40) {
				srcName = selectSource("lead", 2); n.amplitudeDB += 1;
			} else {
				srcName = selectSource("wind", 1); n.amplitudeDB += 5;
				// make higher pitch notes quieter
				n.amplitudeDB -= (n.pitchSemitones-35.)*0.25;
			}
			...
			// counter melody
			srcName = selectSource("wind", 0); n.amplitudeDB += 1; n.pitchSemitones -= .1;
			n.keyframes.clear();
			
			// in second half of the song this channel is the "mountain melody"
			if(n.start.absRow >= 4*64+40)
				n.amplitudeDB -= 6;
			...
			// arps
			srcName = selectSource("wind", 1); n.amplitudeDB -= 8; //n.pitchSemitones += 12;
			//if(n.channel == 101) n.amplitudeDB += 5;
			//if(n.channel == 102) n.amplitudeDB += 5;
			reverbWet = 0.1;
			fade = 0.;
